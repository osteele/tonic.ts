...
export class ChordClass {
  ...
  at(root: Pitch | string): Chord {
    return new Chord({ chordClass: this, root });
  }

  static fromIntervals(intervals: Interval[]): ChordClass {
  ...
  static fromString(name: string): ChordClass {
    let chord = ChordClassMap[name];
    if (!chord) {
...
export class Chord {
  ...
  invert(inversionKey: number | string): Chord {
  ...
  static fromString(name: string): Chord | ChordClass {
    ...
    let [rootName, className] = match.slice(1);
    if (!className) {
      className = 'Major';
    }
...
].map(function({ name, abbrs, intervals }): ChordClass {
  const fullName = name;
  name = name
    .replace(/Major(?!$)/, 'Maj')
    .replace(/Minor(?!$)/, 'Min')
